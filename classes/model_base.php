...
Abstract Class Model_Base {
	...
	public function getFields() {
		...
		foreach($arrayAllFields as $field){
			if(!empty($this->$field)){
				$arraySetFields[$field] = $this->$field;
	
			}
		}
		return $arraySetFields;
	}
	...
	private function _getSelect($select) {
		if(is_array($select)){
			...
			$querySql= "SELECT $fields FROM $this->table" . $querySql;
			return $querySql;
		}		
		return false;
	}
	...
	private function _getResult($sql){
		try{
			...
			$stmt->execute();
			if (!$stmt) {
				trigger_error ('Unable to execute sql  `' . $sql, /*E_USER_NOTICE*/ E_USER_ERROR);
			}
			$rslt = $db->query($sql);
			//var_dump($sql . $stmt);exit;
